...
def test_llvm_graph_from_several_functions(llvm_extractor_fixture):
    verify_data_dir()
    top_dir = os.path.join(os.environ.get('HOME'),
                           '.local',
                           'yacos',
                           'tests')

    filename = os.path.join(top_dir, 'program_1fn_1.c')

    info = llvm_extractor_fixture.GraphFromSource(filename)
    assert len(info.functionInfos) == 1

    filename = os.path.join(top_dir, 'program_2fn.c')

    info2 = llvm_extractor_fixture.GraphFromSource(filename)
...
def test_llvm_graph_visitor(llvm_extractor_fixture):
    ...
    assert visitor.instructions[0:5] == ["alloca",
                                         "alloca",
                                         "store",
                                         "load",
                                         "icmp"]
...
def test_llvm_graph_basicblocks_have_names(llvm_extractor_fixture):
    ...
    assert [bb.name for bb in fn.basicBlocks] == ["val0",
                                                  "val1",
                                                  "val3",
                                                  "val2"]
...
def test_llvm_seq_from_several_functions(llvm_extractor_fixture):
    ...
    info = llvm_extractor_fixture.SeqFromSource(filename)
    assert len(info.functionInfos) == 1

    filename = os.path.join(top_dir, 'program_2fn.c')

    info2 = llvm_extractor_fixture.SeqFromSource(filename)
    assert len(info2.functionInfos) == 2
...
def test_clang_graph_from_several_functions(clang_extractor_fixture):
    ...
    info = clang_extractor_fixture.GraphFromSource(filename)
    assert len(info.functionInfos) == 1

    filename = os.path.join(top_dir, 'program_2fn.c')

    info2 = clang_extractor_fixture.GraphFromSource(filename)
    assert len(info2.functionInfos) == 2
...
def test_clang_graph_functions_have_types_and_names(clang_extractor_fixture):
    ...
    assert [x.name for x in info.functionInfos] == ["max", "foo"]
    assert [x.type for x in info.functionInfos] == ["int (int, int)",
                                                    "int (int)"]
...
def test_clang_seq_from_several_functions(clang_extractor_fixture):
    ...
    filename = os.path.join(top_dir, 'program_1fn_1.c')
    info = clang_extractor_fixture.SeqFromSource(filename)
    assert len(info.functionInfos) == 1

    filename = os.path.join(top_dir, 'program_2fn.c')

    info2 = clang_extractor_fixture.SeqFromSource(filename)
    assert len(info2.functionInfos) == 2

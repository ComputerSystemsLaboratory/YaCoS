...
TEST_F(LLVMExtractorCFixture, ExtractFromFunction1) {
  graph::ExtractionInfoPtr info = extractor_->GraphFromSource("program_k1.c");
...
TEST_F(LLVMExtractorCFixture, ExtractFromFunction2) {
  graph::ExtractionInfoPtr info = extractor_->GraphFromSource("program_k2.c");
...
TEST_F(LLVMExtractorCFixture, ExtractFromFunctionWithSystemInclude) {
  graph::ExtractionInfoPtr info = extractor_->GraphFromSource("program_k3.c");
...
TEST_F(LLVMExtractorCFixture, ExtractFromFunctionWithUserInclude) {
  ...
  graph::ExtractionInfoPtr info = extractor_->GraphFromSource("program_k4.c");
...
TEST_F(LLVMExtractorCFixture, ExtractFromBadFunction) {
  EXPECT_THROW(
      {
        try {
          graph::ExtractionInfoPtr info = extractor_->GraphFromSource("program_foo.c");
...
TEST_F(LLVMExtractorCFixture, ExtractWithDifferentOptimizationlevels) {
  ...
  graph::ExtractionInfoPtr infoO0 = extractor_->GraphFromSource("program_k2.c");

  driver_->setOptimizationLevel(CD::OptimizationLevel::O1);
  graph::ExtractionInfoPtr infoO1 = extractor_->GraphFromSource("program_k2.c");

...
# This code finds similar functions based on numeric features.
#
...
def prepare_training_data(representation_directory,
                          benchmarks):
...
def prepare_test_data(representation_directory,
                      benchmarks):
    """Load the data into a dictonary."""
    representations = {}
    for benchmark in benchmarks:
        ...
        if data:
            ...
            for i, name in enumerate(functions):
                representations[benchmark][name] = [
                    sum(x) for x in zip(*values[i])
                ]
...
def execute(argv):
    ...
    test_data = prepare_test_data(FLAGS.test_representation_directory,
                                  test_benchmarks)

    # Measure the distance
    for test_bench in test_benchmarks:
        index = test_bench.find('.')
        test_suite_name = test_bench[:index]
        test_bench_name = test_bench[index+1:]

        test_function_data = test_data[test_bench]

        if FLAGS.distance == 'euclidean':
            training_index, test_index, distance = Similarity.euclidean_distance_from_data(training_data, test_function_data)
        elif FLAGS.distance == 'manhattan':
            training_index, test_index, distance = Similarity.manhattan_distance_from_data(training_data, test_function_data)
        elif FLAGS.distance == 'cosine':
            training_index, test_index, distance = Similarity.cosine_distance_from_data(training_data, test_function_data)

        test_functions = {}

        functions = 0
        for test_function in test_index:
            test_functions[test_function.item()] = 'function{}'.format(
                                                                    functions
                                                                      )
            functions += 1

        for i, test_function in enumerate(test_index):

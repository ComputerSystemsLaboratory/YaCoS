...
#
# Extract Wu-Larus costs from each functions, and also the data:
#
# - compile time
# - binary size
# - code size
...
def execute(argv):
    ...
    for benchmark in tqdm(benchmarks, desc='Processing'):
        ...
        for key, data in sequences.items():
            ...
            # Compile time
            filename = '{}/compile_time.yaml'.format(bench_dir)
            compile_time = IO.load_yaml(filename)

            # Binary size
            filename = '{}/binary_size.yaml'.format(bench_dir)
            binary_size = IO.load_yaml(filename)

            # Code size
            filename = '{}/code_size.yaml'.format(bench_dir)

...
def extract(benchmark_dir,
    ...
    for source in sources:
        ...
        for functionInfo in extractionInfo.functionInfos:
            representation[module][
                functionInfo.name
            ] = builder.info_to_representation(functionInfo,
                                               visitors[graph_type])
    ...
    for benchmark in tqdm(benchmarks, desc='Processing'):
        ...
        for module, functions in representation.items():
            for function, rep in functions.items():
                filename = '{}/{}_{}.pk'.format(results_dir, module, function)
                IO.dump_pickle(rep.networkx(), filename)

                if FLAGS.dot:
                    rep.draw(path=filename.replace('.pk', '.dot'))

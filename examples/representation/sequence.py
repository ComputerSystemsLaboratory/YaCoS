...
def extract(benchmark_dir,
    ...
    for source in sources:
        ...
        # Build the sequence.
        representation[module] = builder.info_to_representation(extractionInfo,
                                                                visitor)
...
def execute(argv):
    ...
    for benchmark in tqdm(benchmarks, desc='Processing'):
        ...
        for module, rep in representation.items():
            filename = '{}/{}.dot'.format(results_dir, module)
            rep.draw(limit=100, path=filename)

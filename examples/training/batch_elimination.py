...
from yacos.algorithms import BatchElimination
...
def execute(argv):
    ...
    be = BatchElimination(Goals.prepare_goals(FLAGS.goals, FLAGS.weights),
                          'opt',
    ...
    for benchmark in tqdm(benchmarks, desc='Processing'):
        ...
        # Run the algorithm.
        be.run(benchmark, sequences)

        if be.results:
            # Store the results
            IO.dump_yaml(be.results, filename)

...
from yacos.algorithms import IterativeElimination


def execute(argv):
    """Execute Iterative Elimination."""
    del argv
    ...
    ie = IterativeElimination(Goals.prepare_goals(FLAGS.goals, FLAGS.weights),
                              'opt',
    ...
    for benchmark in tqdm(benchmarks, desc='Processing'):
        ...
        # Run the algorithm.
        ie.run(benchmark, sequences)

        if ie.results:
            # Store the results
            IO.dump_yaml(ie.results, filename)

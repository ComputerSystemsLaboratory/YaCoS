...
from yacos.algorithms import CombinedElimination


def execute(argv):
    """Execute Combined Elimination."""
    del argv
    ...
    # Initialize a CombinedElimination object.
    ce = CombinedElimination(Goals.prepare_goals(FLAGS.goals, FLAGS.weights),
                             'opt',
    ...
    for benchmark in tqdm(benchmarks, desc='Processing'):
        ...
        # Run the algorithm.
        ce.run(benchmark, sequences)

        if ce.results:
            # Store the results
            IO.dump_yaml(ce.results, filename)
